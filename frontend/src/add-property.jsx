...
const AddProperty = ({ navigate, onCreate }) => {
  const { CITIES, AMENITIES, TYPES } = window.DATA;
  ...
  const validateStep = () => {
    const e = {};
    if (step === 1) {
      if (!data.type) e.type = "Escolha o tipo do imóvel.";
          ...
          <div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                {TYPES.map((t) => (
                  <button
                    ...
                    className="btn ghost"
                    style={{
                      borderColor: data.type === t ? "var(--ink)" : "var(--line-2)",
                      background: data.type === t ? "var(--bg-2)" : "transparent",
                    }}
                  >
                    {t}
                  </button>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 28 }}>
              ...
              <div className="field">
                ...
                <input className={"input" + (errors.neighborhood ? " invalid" : "")} value={data.neighborhood} onChange={(e) => update("neighborhood", e.target.value)} placeholder="Ex: Vila Madalena" />
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18, marginTop: 28 }}>
              <div className="field">
                ...
                <input className={"input" + (errors.title ? " invalid" : "")} value={data.title} onChange={(e) => update("title", e.target.value)} placeholder="Ex: Apartamento amplo na Vila Madalena" maxLength={80} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                <div className="field">
                  ...
                  <input className={"input" + (errors.price ? " invalid" : "")} type="number" value={data.price} onChange={(e) => update("price", e.target.value)} placeholder="3500" />
              ...
              <div className="field">
                ...
                <div className="checkgrid">
                  {AMENITIES.map((a) => {
                    ...
                    return (
                      <label key={a} className={on ? "checked" : ""}>
                        <input type="checkbox" checked={on} onChange={() => toggleAmenity(a)} />
                        {a}
                      </label>
                    );
                  })}
        ...
        {step === 4 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 32, marginTop: 28 }}>
              ...
              <div>
                <span className="pill sun">{data.type}</span>
                <h3 style={{ fontSize: 22, marginTop: 10, lineHeight: 1.15 }}>{data.title}</h3>
                <p className="muted" style={{ fontSize: 13, marginTop: 6 }}>
                  <Icon name="pin" size={12} /> {data.neighborhood}, {data.city}
                </p>
                ...
                {data.amenities.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 16 }}>
                    {data.amenities.map((a) => <span key={a} className="pill">{a}</span>)}
                  </div>
                )}
              </div>
            </div>
